...
class Producer(val topic: String, val isAsync: Boolean) extends Thread {
  ...
  props.put(ProducerConfig.VALUE_SERIALIZER_CLASS_CONFIG, classOf[AccessLogRecordSerializer].getName)

  private val producer = new KafkaProducer[Integer, AccessLogRecord](props)
  ...
  override def run(): Unit = {
    ...
    for (value <- filteredLines) {
      val messageStr = value
      val startTime = System.currentTimeMillis
      if (isAsync) { // Send asynchronously
        producer.send(new ProducerRecord[Integer, AccessLogRecord](topic, messageNo, messageStr), new DemoCallBack(startTime, messageNo, messageStr))
      }
      else { // Send synchronously
        try {
          producer.send(new ProducerRecord[Integer, AccessLogRecord](topic, messageNo, messageStr)).get
...
class DemoCallBack(val startTime: Long, val key: Int, val message: AccessLogRecord) extends Callback {
